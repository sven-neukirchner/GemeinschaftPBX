...
<?
  ...
  $mdisplay  = date('m', $t);                                // month of selected  month
  $day_t_start = (int)mkTime(  0, 0, 0 , $m,1,$y );
  $day_t_end   = (int)mkTime( 23,59,59 , $m,$num_days,$y );
...
<?
 //echo "<b>Erster Tag :</b> ". date('d',$day_t_start) .".". date('m',$day_t_start) ."<b>&nbsp;&nbsp;Letzter Tag:</b> ". date('d',$day_t_end) .".". date('m',$day_t_end) ."<br>";
?>
...
echo '<a href="', GS_URL_PATH, 'srv/csv-fsc-month-export.php?d_start='.$day_t_start.'&d_end='.$day_t_end.'" title="', __('CSV Export b '), '">'.__('CSV Export').'</a>';
?>
