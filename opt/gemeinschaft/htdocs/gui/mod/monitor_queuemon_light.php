...
require_once( GS_DIR .'inc/mongr_light.php');
...
$day_start	= (int)mkTime(  0, 0, 0 , $month,$day,$year );
$day_end	= (int)mkTime( 23,59,59 , $month,$day,$year );
$sql_today	= '(`timestamp`>='.$day_start .' AND `timestamp`<='.$day_end .')';
...
function queue_timeout($queue_id, $last_min)
{
	...
	($last_min == 11111) ? ($sql_time = $sql_today) : ($sql_time = '(`timestamp` >= '.(time() - $last_min * 60).')');
...
<?php
	...
	foreach ($queues as $queue => $queue_data) {
		...
		if ($queue_data['display_timeout'])		$stats[] = '&#9719; '. queue_timeout($queue_data['id'], $queue_data['display_timeout']);
		...
		if ($queue_data['display_calls'])		$tooltip[] = 'calls';
		if ($queue_data['display_answered'])	$tooltip[] = 'answered';
		if ($queue_data['display_abandoned'])	$tooltip[] = 'abandoned';
		if ($queue_data['display_timeout'])		$tooltip[] = 'timeout';
		if ($queue_data['display_wait_max'])	$tooltip[] = 'wait_max';
		if ($queue_data['display_wait_min'])	$tooltip[] = 'wait_min';
		if ($queue_data['display_wait_avg'])	$tooltip[] = 'wait_avg';
		if ($queue_data['display_call_max'])	$tooltip[] = 'call_max';
		if ($queue_data['display_call_min'])	$tooltip[] = 'call_min';
		if ($queue_data['display_call_avg'])	$tooltip[] = 'call_avg';

		$st_x = $qx;
		$st_y = $qy;
		
		if ($queue_data['active']) {
				queue_window($st_x,$st_y,$queue_data['display_width'],$queue_data['display_height'],"q".$queue,$queue_data['title'], $queue_data['members'], $queue_data['display_columns'], 0, CQ_WINDOW_BG, CQ_WINDOW_FG, $stats, $tooltip);
